...
function TravellerDetails() {
  ...
  // If booking details are missing, redirect to homepage
  useEffect(() => {
    if (!booking) {
  ...
  const submitTravelerDetails = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/booking/${bookingId}/travelers`, {
        method: "POST",
    ...
    <>
      ...
      <div className="container travellerBorder mb-5">
        <div className="safaridatetime mt-3">
          <h6>
            Safari Date: {date ? new Date(date).toLocaleDateString() : "N/A"} | Safari Time: {safariTime || "N/A"}
          </h6>
        </div>
        ...
        {travelerDetails.map((traveler, index) => (
          <div className="d-flex justify-content-around mt-2" key={index}>
            <div>
              <p>{index + 1}. {index < adults ? "Adult" : "Child"}</p>
            </div>
            <div>
              <input
                type="text"
            ...
            <div>
              <select value={traveler.nationality} onChange={(e) => handleTravelerChange(index, "nationality", e.target.value)}>
                <option value="Indian">Indian</option>
                <option value="Foreigner">Foreigner</option>
              </select>
            </div>
            <div>
              <select value={traveler.idType} onChange={(e) => handleTravelerChange(index, "idType", e.target.value)}>
        ...
        <button className="payable mt-2" onClick={submitTravelerDetails}>
        ...
        <div className="mt-4">
          <table className="table w-100 table-bordered">
            <thead>
              <tr>
                <th className="bookingtable" colSpan={4}>Booking Summary</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td><b>Safari Type:</b></td>
                <td>{vehicleType || "N/A"}</td>
              </tr>
              <tr>
                <td><b>Safari Timing:</b></td>
        ...
        {/* Declaration */}
        <div>
          <p className="bookingtable"><b>Declaration</b></p>
          <p>
            I authorize Safari for using my Mobile Number, Identity Proof & Email mentioned in this permit for intimation and verification purposes.
          </p>
          <ul>
            <li>All reservations inside the Tadoba National Park are provisional and can be changed or cancelled without prior information.</li>
